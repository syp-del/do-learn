...
- **`index.html`**: 앱 전체.
  ...
  - 🎈 놀이: 부모님이 승인해야 열린다. 🧠 생각 놀이 4가지는 직접 만든 문제 생성기라서 매번 새 문제가 나온다.
    ...
    - 🌱 이또 `it*`(층 `#itLayer`, 상태 `IT`): 정원 풀을 문질러 깎으면(캔버스 `destination-out`) 숨은 친구가 나타난다 → 땅속 탐험차(4자리)에 태워 → 🚗 출발 → 땅속 동굴의 버블에 옮겨 담고 같이 논다 → 🌱 정원으로. 친구 8종(`IT_FRIENDS`, 반짝이·달님이는 드묾)은 이 앱에서 새로 그린 캐릭터다. 동굴·차·도감은 `profile.itto`.
    - 🧸 곰돌이 숨바꼭질 `bh*`(층 `#bhLayer`, 상태 `BH`): 키즈카페 9곳(`BH_SPOTS`·`BH_ART`)에서 곰돌이가 나타났다 숨는다. 빠르게 눌러야 해서 `pointerdown`으로 받는다. 한 단계 60초, 목표 15·20·25…(`bhParams`), 목표를 넘으면 🏆 + 다음 단계는 더 빨리·더 많이. 반짝 곰돌이는 +2. 트로피·최고 단계는 `profile.bear`. 화면을 숨기면 멈춘다.
